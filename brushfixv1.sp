...
// Defines
// ===================================================================================================================================
#define 	PLUGIN_VERSION 		"0.1.3 FiX"
...
#define MAX_CTS 3
#define CHAT_TAG "\x04[BRush]\x01"
#define CHAT_DEFAULT "\x01"
#define CHAT_T "\x07FF0000"
#define CHAT_CT "\x070000FF"
...
new PlayerKilledCT[MAXPLAYERS+1]; // Holds the number of CTs the T killed
new ChangeTeamImmune[MAXPLAYERS+1]; // Marks the player as Change Team Immune
new SwitchingPlayer[MAXPLAYERS+1]; // Flags the player as being switched by the plugin
new bool:PlayerSwitchable[MAXPLAYERS+1]; // Flags the player as switchable
...
public OnPluginStart() {
	...
	"Version of 'CSS BRush'", FCVAR_SPONLY | FCVAR_NOTIFY | FCVAR_DONTRECORD)), OnVersionChanged);
...
public Action Timer_AutoAssignTeam(Handle timer, any userId)
{
    ...
    if(GameIsLive)
    {
        if(tCount < 5)
        {
            ...
            BR_PrintToChat(client, "%s You have been assigned to %sTerrorists%s", CHAT_TAG, CHAT_T, CHAT_DEFAULT);
        }
        else if(ctCount < 3)
        {
            ...
            BR_PrintToChat(client, "%s You have been assigned to %sCounter-Terrorists%s", CHAT_TAG, CHAT_CT, CHAT_DEFAULT);
        }
        else
        {
            CS_SwitchTeam(client, CS_TEAM_SPECTATOR);
            BR_PrintToChat(client, "%s Teams are full! (T: %d/5, CT: %d/3)", CHAT_TAG, tCount, ctCount);
            CreateTimer(0.1, Timer_CheckTeamsAfterDisconnect);
        }
        return Plugin_Stop;
    }
    ...
    if(tCount == 0 && ctCount == 0)
    {
        CS_SwitchTeam(client, CS_TEAM_T);
        CS_RespawnPlayer(client);
        BR_PrintToChat(client, "%s You are the first player and assigned to %sTerrorists%s", CHAT_TAG, CHAT_T, CHAT_DEFAULT);
    }
    else if(tCount < 5)
    {
        CS_SwitchTeam(client, CS_TEAM_T);
...
stock int SwitchPlayerTeam(int client, int team) {
    #if _DEBUG
...
public Event_RoundFreezeEnd(Handle:event, const String:name[], bool:dontBroadcast) {
    ...
    // Если игра уже началась хотя бы раз, проверяем минимальные требования
    if (g_bGameStarted) {
        if (tCount >= 3 && ctCount >= 2) {
            GameIsLive = true;
            ...
            GameIsLive = false;
            Format(BR_msg, sizeof(BR_msg), "%t", "NotEnoughPlayers");
            BR_PrintToChatAll(BR_msg);
        }
    }
...
{
    return (client > 0 && client <= MaxClients && IsClientConnected(client) && IsClientInGame(client) && !IsClientSourceTV(client) && !IsClientReplay(client));
}

...
public Action Timer_CheckForSpectators(Handle timer)
{
    ...
    // Если T > 5, не показываем меню наблюдателям
    if(tCount > 5)
        return Plugin_Stop;
...
void ShowSlotAvailableMenu(int client)
{
    ...
    // Если в T больше 5 игроков - блокируем выбор команд для наблюдателей
    if(tCount > 5)
    {
    ...
    // Иначе показываем доступные слоты
    if(ctCount < 3)
